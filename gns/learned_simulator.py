...
class LearnedSimulator(nn.Module):
  ...
  def merge_world_mesh_graph(self, mesh_index, world_index):
    ...
    Args:
      mesh_index: the mesh graph edge index, generated by the mesh at rest stage
      world_index: the world graph edge index, this is based on particle within the threshold radius
    '''
  ...
  def get_mesh_displacement(self, senders, receivers):
    '''
    if the particle is a mesh particle, then compute the displacement, otherwise 
    return a large value: 100

    one batch contains multiple examples, each example contains two type of 
    particles: mesh particle & obstacle particle. the starting index of mesh example 
    is always at 0 position of the batch, the ending index is self.mesh_size
    '''
    bc = self.batch_ids.bincount()[0] #first batch size
    s = (senders % bc)    # sender index in the batch
    r = (receivers % bc)  # receiver index in the batch

    # convert the mesh index to the mesh coordinate, then compute the displacement
    # x = s % mesh_width, y = s // mesh_width
    d_x = torch.where( torch.logical_and(s < self.mesh_size, r < self.mesh_size), 
                      s % mesh_width - r % mesh_width, 100)
    d_y = torch.where( torch.logical_and(s < self.mesh_size, r < self.mesh_size), 
                      s // mesh_width - r // mesh_width, 100)

    # convert index coordinate to the world coordinate by multiplying the quad 
    # size, the quad size is the distance between two adjacent mesh points. Then 
    # normalize the displacement by the connectivity radius
    self._normalized_relative_displacements = \
      torch.cat([d_x.reshape(-1,1), d_y.reshape(-1,1)], axis=1) \
      * self._quad_size /self._connectivity_radius

    return self._normalized_relative_displacements # (nedges, 2)

  def get_mesh_distance(self, senders, receivers):
    '''
    compute the distance between mesh particles when the cloth is in rest stage (2D)
    
    '''
    mesh_dist = torch.norm(self.get_mesh_displacement(senders, receivers), dim=1)
    return mesh_dist.reshape(-1, 1) # (nedges, 1)
